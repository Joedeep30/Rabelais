...
function processContent(content) {
    let safeContent = content.replace(/class=/g, 'className=');
    safeContent = safeContent.replace(/style="[^"]*"/g, '');
    safeContent = safeContent.replace(/style='[^']*'/g, '');
    return safeContent;
}
...
async function processAll() {
    ...
    // Use a JS template string to avoid string escaping issues
...
export default function PathologiesOculairesPage() {
  const customHtml = \`${contentPage.replace(/`/g, '\\`').replace(/\$/g, '\\$')}\`;
