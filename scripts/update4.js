...
const pubs = [
    ...
    {"title": "Is the \\"dark rim\\" of classic choroidal neovascularisation a reliable anatomic landmark?", "file": "dark-rim-classic-choroidal-1995"}
];
...
for (const p of pubs) {
  // Use a regex to find the block for this title
  // Looking for: title: "Escaped title" -> up to next 'imageUrl:'
  const escapedTitle = p.title.replace(/[.*+?^$\\{\\}()|[\\]\\\\]/g, '\\\\$&');
  const rx = new RegExp(`title:\\s*["']${escapedTitle}["'][\\\\s\\\\S]*?imageUrl:\\s*["']([^"']+)["']`);
  conMatch = c.match(rx);
  if (conMatch) {
    const oldUrl = conMatch[1];
    const newUrl = `/images/publications/thumb_${p.file}.png`;
    c = c.replace(conMatch[0], conMatch[0].replace(oldUrl, newUrl));
    replaced++;
  } else {
    // If exact match fails, try partial match (first 20 chars)
